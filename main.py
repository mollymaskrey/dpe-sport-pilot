...
- Ask "As a Sport Pilot, can you..." questions frequently.
    ...
    splash = html.Div(id="splash-screen", style=SHOW, children=[
        ...
        html.Div(className="tron-wrap", children=[
            ...
            html.Div(className="auth-panel", children=[
                ...
                html.Button("Begin Oral Exam →", id="splash-btn",
...
# ---------------------------------------------------------------------------
# Callback: splash → exam (toggle display)
# ---------------------------------------------------------------------------
...
def handle_splash(n_clicks, n_submit, name):
    if not name or not name.strip():
        return no_update, no_update, no_update, "// enter your name to begin", "msg-denied"
    pilot = name.strip().title()
    return HIDE, SHOW, pilot, no_update, no_update
...
def handle_interaction(start_clicks, send_clicks, user_text,
    ...
    if trigger == "start-btn":
        ...
        return conversation, meta, bubbles, banner, False, False, "New Session", ""
    ...
    if trigger == "send-btn" and user_text and user_text.strip():
        ...
        return conversation, meta, bubbles, no_update, False, False, no_update, ""
